...
/**
 * This is the model class for table "realisasi_rkt_infra_mukim".
 *
 * The followings are the available columns in table 'realisasi_rkt_infra_mukim':
 * @property integer $id
 * @property integer $id_rkt_infra_mukim
 ...
 * @property RktInfraMukim $idRktInfraMukim
 * @property MasterBulan $idBulan
 */
class RealisasiRktInfraMukim extends CActiveRecord
{
	/**
	...
	{
		return 'realisasi_rkt_infra_mukim';
	}
	...
	public function rules()
	{
		...
		return array(
			array('id_rkt_infra_mukim, id_bulan, realisasi, persentase, created, updated', 'required'),
			array('id_rkt_infra_mukim, id_bulan', 'numerical', 'integerOnly'=>true),
			array('tahun', 'length', 'max'=>4),
			array('realisasi, persentase', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_rkt_infra_mukim, id_bulan, tahun, realisasi, persentase, created, updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'idRktInfraMukim' => array(self::BELONGS_TO, 'RktInfraMukim', 'id_rkt_infra_mukim'),
	...
	public function attributeLabels()
	{
		return array(
						'id'=>Yii::t('app','ID'),
						'id_rkt_infra_mukim'=>Yii::t('app','Id Rkt Infra Mukim'),
	...
	public function search()
	{
		...
		$criteria->compare('id_rkt_infra_mukim',$this->id_rkt_infra_mukim);
	 ...
	 * @return RealisasiRktInfraMukim the static model class
	 */
	...
	public function searchByRkt()
	{
		...
		$criteria->with = array('idRktInfraMukim');
		$criteria->compare('idRktInfraMukim.id_rkt',$this->id_rkt);
		...
		$criteria->join = 'LEFT JOIN '. $this->tableName() .' bulan_lalu ON
		 	bulan_lalu.id_rkt_infra_mukim = t.id_rkt_infra_mukim AND
		...
		$criteria->group = 't.id_rkt_infra_mukim';
		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
	...
	public function getTotal($records, $colName){
        ...
        foreach ($records as $rec){
			if ($colName == 'jumlah'){
				$total+=$rec->idRktInfraMukim->jumlah;
			}else{
				$total+=$rec->{$colName};
			}
		}
	...
	public function getTotalPersen($records, $colName){
    	...
    	$totalRealisasi = 0;
    	foreach($records as $rec){
    		$totalJumlah += $rec->idRktInfraMukim->jumlah;

...
/**
 * This is the model class for table "realisasi_rkt_tata_batas".
 *
 * The followings are the available columns in table 'realisasi_rkt_tata_batas':
 * @property integer $id
 * @property integer $id_rkt_tata_batas
 ...
 * @property RktTataBatas $idRktTataBatas
 * @property MasterBulan $idBulan
 */
class RealisasiRktTataBatas extends CActiveRecord
{
	/**
	...
	{
		return 'realisasi_rkt_tata_batas';
	}
	...
	public function rules()
	{
		...
		return array(
			array('id_rkt_tata_batas, id_bulan, realisasi, persentase, created, updated', 'required'),
			array('id_rkt_tata_batas, id_bulan', 'numerical', 'integerOnly'=>true),
			array('tahun', 'length', 'max'=>4),
			array('realisasi, persentase', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_rkt_tata_batas, id_bulan, tahun, realisasi, persentase, created, updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'idRktTataBatas' => array(self::BELONGS_TO, 'RktTataBatas', 'id_rkt_tata_batas'),
	...
	public function attributeLabels()
	{
		return array(
						'id'=>Yii::t('app','ID'),
						'id_rkt_tata_batas'=>Yii::t('app','Id Rkt Tata Batas'),
	...
	public function search()
	{
		...
		$criteria->compare('id_rkt_tata_batas',$this->id_rkt_tata_batas);
	 ...
	 * @return RealisasiRktTataBatas the static model class
	 */
	...
	public function searchByRkt()
	{
		...
		$criteria->with = array('idRktTataBatas');
		$criteria->compare('idRktTataBatas.id_rkt',$this->id_rkt);
		...
		$criteria->join = 'LEFT JOIN '. $this->tableName() .' bulan_lalu ON
		 	bulan_lalu.id_rkt_tata_batas = t.id_rkt_tata_batas AND
		...
		$criteria->group = 't.id_rkt_tata_batas';
		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
	...
	public function getTotal($records, $colName){
        ...
        foreach ($records as $rec){
			if ($colName == 'jumlah'){
				$total+=$rec->idRktTataBatas->jumlah;
			}else{
				$total+=$rec->{$colName};
			}
		}
	...
	public function getTotalPersen($records, $colName){
		...
		$idx = 0;
    	foreach($records as $rec){
    		$totalJumlah += $rec->idRktTataBatas->jumlah;
    		$totalRealisasi +=$rec->$colName;
			if($rec->idRktTataBatas->jumlah > 0) {
				$idx++;
			}
    	}

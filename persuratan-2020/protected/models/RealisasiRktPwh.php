...
/**
 * This is the model class for table "realisasi_rkt_pwh".
 *
 * The followings are the available columns in table 'realisasi_rkt_pwh':
 * @property integer $id
 * @property integer $id_rkt_pwh
 ...
 * @property RktPwh $idRktPwh
 * @property MasterBulan $idBulan
 */
class RealisasiRktPwh extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('id_rkt_pwh, id_bulan, realisasi, persentase, created, updated', 'required'),
			array('id_rkt_pwh, id_bulan', 'numerical', 'integerOnly'=>true),
			array('tahun', 'length', 'max'=>4),
			array('realisasi, persentase', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_rkt_pwh, id_bulan, tahun, realisasi, persentase, created, updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'idRktPwh' => array(self::BELONGS_TO, 'RktPwh', 'id_rkt_pwh'),
	...
	public function attributeLabels()
	{
		return array(
						'id'=>Yii::t('app','ID'),
						'id_rkt_pwh'=>Yii::t('app','Id Rkt Pwh'),
	...
	public function search()
	{
		...
		$criteria->compare('id_rkt_pwh',$this->id_rkt_pwh);
	 ...
	 * @return RealisasiRktPwh the static model class
	 */
	...
	public function searchByRkt()
	{
		...
		$criteria->with = array('idRktPwh');
		$criteria->compare('idRktPwh.id_rkt',$this->id_rkt);
		...
		$criteria->join = 'LEFT JOIN '. $this->tableName() .' bulan_lalu ON
		 	bulan_lalu.id_rkt_pwh = t.id_rkt_pwh AND
	...
	public function getTotalPersen($records, $colName){
		...
		$idx = 0;
    	foreach($records as $rec){
    		$totalJumlah += $rec->idRktPwh->jumlah;
    		$totalRealisasi +=$rec->$colName;
			if($rec->idRktPwh->jumlah > 0) {
				$idx++;
			}
    	}

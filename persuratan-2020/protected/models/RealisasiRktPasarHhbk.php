 ...
 */
class RealisasiRktPasarHhbk extends CActiveRecord
{
	/**
	...
	{
		return 'realisasi_rkt_pasar_hhbk';
	}
	...
	public function rules()
	{
		...
		return array(
			array('id_rkt_pasar_hhbk, id_bulan, realisasi, persentase, created, updated', 'required'),
			array('id_rkt_pasar_hhbk, id_bulan', 'numerical', 'integerOnly'=>true),
			array('tahun', 'length', 'max'=>4),
			array('realisasi, persentase', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_rkt_pasar_hhbk, id_bulan, tahun, realisasi, persentase, created, updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'idRktPasarHhbk' => array(self::BELONGS_TO, 'RktPasarHhbk', 'id_rkt_pasar_hhbk'),
	...
	public function attributeLabels()
	{
		return array(
						'id'=>Yii::t('app','ID'),
						'id_rkt_pasar_hhbk'=>Yii::t('app','Id Rkt Pasar HHBK'),
	...
	public function search()
	{
		...
		$criteria->compare('id_rkt_pasar_hhbk',$this->id_rkt_pasar_hhbk);
	...
	public function searchByRkt()
	{
		...
		$criteria->with = array('idRktPasarHhbk');
		$criteria->compare('idRktPasarHhbk.id_rkt',$this->id_rkt);
		...
		$criteria->join = 'LEFT JOIN '. $this->tableName() .' bulan_lalu ON
		 	bulan_lalu.id_rkt_pasar_hhbk = t.id_rkt_pasar_hhbk AND
		...
		$criteria->group = 't.id_rkt_pasar_hhbk';
		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
	...
	public function getTotal($records, $colName){
        ...
        foreach ($records as $rec){
			if ($colName == 'jumlah'){
				$total+=$rec->idRktPasarHhbk->jumlah;
			}else{
				$total+=$rec->{$colName};
			}
		}
	...
	public function getTotalPersen($records, $colName){
    	...
    	$totalRealisasi = 0;
    	foreach($records as $rec){
    		$totalJumlah += $rec->idRktPasarHhbk->jumlah;
    ...
    public function getByRktAndBulan($rktId,$idBulan)
    {
        $idBulan = $idBulan+1;
            ...
            $query = "SELECT b.id,
                ...
                IFNULL((SELECT m.nama_pemasaran FROM master_jenis_pemasaran m WHERE m.id = b.id_jenis_pasar),0)AS jenispemasaran,
                b.jumlah AS target,
                IFNULL((SELECT SUM(r.realisasi) FROM realisasi_rkt_pasar r WHERE r.id_rkt_pasar=b.id AND r.id_bulan < ".$idBulan."),0)AS realisasi
                FROM rkt_pasar_hhbk b WHERE b.id_rkt=".$rktId;

...
/**
 * This is the model class for table "realisasi_rkt_panen_areal".
 *
 * The followings are the available columns in table 'realisasi_rkt_panen_areal':
 * @property integer $id
 * @property integer $id_rkt_panen_areal
 ...
 * @property RktPanenAreal $idRktPanenAreal
 * @property MasterBulan $idBulan
 */
class RealisasiRktPanenAreal extends CActiveRecord
{
	/**
	...
	{
		return 'realisasi_rkt_panen_areal';
	}
	...
	public function rules()
	{
		...
		return array(
			array('id_rkt_panen_areal, id_bulan, realisasi, persentase, created, updated', 'required'),
			array('id_rkt_panen_areal, id_bulan', 'numerical', 'integerOnly'=>true),
			array('tahun', 'length', 'max'=>4),
			array('realisasi, persentase', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_rkt_panen_areal, id_bulan, tahun, realisasi, persentase, created, updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'idRktPanenAreal' => array(self::BELONGS_TO, 'RktPanenAreal', 'id_rkt_panen_areal'),
	...
	public function attributeLabels()
	{
		return array(
						'id'=>Yii::t('app','ID'),
						'id_rkt_panen_areal'=>Yii::t('app','Id Rkt Panen Areal'),
	...
	public function search()
	{
		...
		$criteria->compare('id_rkt_panen_areal',$this->id_rkt_panen_areal);
	 ...
	 * @return RealisasiRktPanenAreal the static model class
	 */
	...
	public function searchByRkt()
	{
		...
		$criteria->with = array('idRktPanenAreal');
		$criteria->compare('idRktPanenAreal.id_rkt',$this->id_rkt);
		...
		$criteria->join = 'LEFT JOIN '. $this->tableName() .' bulan_lalu ON
		 	bulan_lalu.id_rkt_panen_areal = t.id_rkt_panen_areal AND
		...
		$criteria->group = 't.id_rkt_panen_areal';
		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
	...
	public function getTotal($records, $colName){
        ...
        foreach ($records as $rec){
			if ($colName == 'jumlah'){
				$total+=$rec->idRktPanenAreal->jumlah;
			}else{
				$total+=$rec->{$colName};
			}
		}
	...
	public function getTotalPersen($records, $colName){
    	...
    	$totalRealisasi = 0;
    	foreach($records as $rec){
    		$totalJumlah += $rec->idRktPanenAreal->jumlah;

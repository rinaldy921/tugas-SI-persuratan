...
/**
 * This is the model class for table "realisasi_rkt_ganis".
 *
 * The followings are the available columns in table 'realisasi_rkt_ganis':
 * @property integer $id
 * @property integer $id_rkt_ganis
 ...
 * @property RktGanis $idRktGanis
 * @property MasterBulan $idBulan
 */
class RealisasiRktGanis extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('id_rkt_ganis, id_bulan, realisasi, persentase, created', 'required'),
			array('id_rkt_ganis, id_bulan', 'numerical', 'integerOnly'=>true),
			array('tahun', 'length', 'max'=>4),
			array('realisasi, persentase', 'length', 'max'=>10),
			array('updated', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_rkt_ganis, id_bulan, tahun, realisasi, persentase, created, updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'idRktGanis' => array(self::BELONGS_TO, 'RktGanis', 'id_rkt_ganis'),
	...
	public function attributeLabels()
	{
		return array(
						'id'=>Yii::t('app','ID'),
						'id_rkt_ganis'=>Yii::t('app','Id Rkt Ganis'),
	...
	public function search()
	{
		...
		$criteria->compare('id_rkt_ganis',$this->id_rkt_ganis);
	 ...
	 * @return RealisasiRktGanis the static model class
	 */
	...
	public function searchByRkt()
	{
		...
		$criteria->with = array('idRktGanis');
		$criteria->compare('idRktGanis.id_rkt',$this->id_rkt);
		...
		$criteria->join = 'LEFT JOIN '. $this->tableName() .' bulan_lalu ON
		 	bulan_lalu.id_rkt_ganis = t.id_rkt_ganis AND
	...
	public function getTotalPersen($records, $colName){
		...
		$idx = 0;
    	foreach($records as $rec){
    		$totalJumlah += $rec->idRktGanis->jumlah;
    		$totalRealisasi +=$rec->$colName;
			if($rec->idRktGanis->jumlah > 0) {
				$idx++;
			}
    	}

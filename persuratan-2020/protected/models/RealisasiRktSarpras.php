...
/**
 * This is the model class for table "realisasi_rkt_sarpras".
 *
 * The followings are the available columns in table 'realisasi_rkt_sarpras':
 * @property integer $id
 * @property integer $id_rkt_sarpras
 ...
 * @property RktSarpras $idRktSarpras
 * @property MasterBulan $idBulan
 */
class RealisasiRktSarpras extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('id_rkt_sarpras, id_bulan, realisasi, persentase, created, updated', 'required'),
			array('id_rkt_sarpras, id_bulan', 'numerical', 'integerOnly'=>true),
			array('tahun', 'length', 'max'=>4),
			array('realisasi, persentase', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_rkt_sarpras, id_bulan, tahun, realisasi, persentase, created, updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'idRktSarpras' => array(self::BELONGS_TO, 'RktSarpras', 'id_rkt_sarpras'),
	...
	public function attributeLabels()
	{
		return array(
						'id'=>Yii::t('app','ID'),
						'id_rkt_sarpras'=>Yii::t('app','Id Rkt Sarpras'),
	...
	public function search()
	{
		...
		$criteria->compare('id_rkt_sarpras',$this->id_rkt_sarpras);
	 ...
	 * @return RealisasiRktSarpras the static model class
	 */
	...
	public function searchByRkt()
	{
		...
		$criteria->with = array('idRktSarpras');
		$criteria->compare('idRktSarpras.id_rkt',$this->id_rkt);
		...
		$criteria->join = 'LEFT JOIN '. $this->tableName() .' bulan_lalu ON
		 	bulan_lalu.id_rkt_sarpras = t.id_rkt_sarpras AND
		...
		$criteria->group = 't.id_rkt_sarpras';
		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
	...
	public function getTotalPersen($records, $colName){
		...
		$idx = 0;
    	foreach($records as $rec){
    		$totalJumlah += $rec->idRktSarpras->jumlah;
    		$totalRealisasi +=$rec->$colName;
			if($rec->idRktSarpras->jumlah > 0) {
				$idx++;
			}
    	}

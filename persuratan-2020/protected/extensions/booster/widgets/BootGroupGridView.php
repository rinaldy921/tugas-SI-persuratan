...
class BootGroupGridView extends TbGridView {
 //column values are merged independently
    ...
    */
    ...
    {
        $data = $this->dataProvider->data[$row]; 
        if($this->extraNipzExpression) {
            $content = $this->evaluateExpression($this->extraNipzExpression, array('data'=>$data, 'row'=>$row, 'totals' => $totals));
            // var_dump($content);die;
            echo $content;
        } elseif($this->extraRowExpression) { //user defined expression, use it!

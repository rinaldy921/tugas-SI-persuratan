...
class RktPembibitanController extends Controller {
    ...
    public function actionFormRktMatch($rku) {
        $ModelRKU = RkuBibit::model()->findByPk($rku);
        ...
        $ModelRKT = new RktBibit();

        if (isset($_POST['RktBibit'])) {
            $ModelRKT->attributes = $_POST['RktBibit'];
    ...
    public function actionPilihRKU($tahun) {
        ...
        $model = new RkuBibit('searchByRkt');
...
//            $model->attributes = $_POST['RktBibit'];
            ...
            $rktRef = Rkt::model()->find(array('condition' => 'status = 1 AND id_perusahaan = ' . Yii::app()->user->idPerusahaan() . ' AND id_rku = ' . $rku->id_rku . " AND rkt_ke = " .  $_POST['RktBibit']['rkt_ke_new']));

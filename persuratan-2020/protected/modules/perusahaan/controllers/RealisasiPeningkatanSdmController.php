...
class RealisasiPeningkatanSdmController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
       $rku = Rku::model()->find(array(
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktPeningkatanSdm::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktPeningkatanSdm::model()->findByAttributes(array(
						'id_rkt_peningkatan_sdm'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
                                            
					));
					if(!$is_exist){
						$realisasi = new RealisasiRktPeningkatanSdm;
						$realisasi->id_rkt_peningkatan_sdm = $value->id;
            ...
            $modelRalisasi = new RealisasiRktPeningkatanSdm;
            $modelRalisasi->unsetAttributes();
            if(isset($_GET['RealisasiRktPeningkatanSdm']))
                $modelRalisasi->attributes = $_GET['RealisasiRktPeningkatanSdm'];
    ...
    public function actionInputRealisasi(){
		...
		$realisasi = RealisasiRktPeningkatanSdm::model()->findByPk($post);
        $_md = RktPeningkatanSdm::model()->findByPk($realisasi->id_rkt_peningkatan_sdm);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktPeningkatanSdm');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_peningkatan_sdm',$realisasi->id_rkt_peningkatan_sdm);
			$getRealisasi = RealisasiRktPeningkatanSdm::model()->find($criteria);

...
class RealisasiLingkunganDungtanController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktLingkunganDungtan::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktLingkunganDungtan::model()->findByAttributes(array(
						'id_rkt_lingkungan_dungtan'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
					));
					if(!$is_exist){
						$realisasiBibit = new RealisasiRktLingkunganDungtan;
						$realisasiBibit->id_rkt_lingkungan_dungtan = $value->id;
            ...
            $modelRealisasi = new RealisasiRktLingkunganDungtan;
            $modelRealisasi->unsetAttributes();
            if(isset($_GET['RealisasiRktLingkunganDungtan']))
                $modelRealisasi->attributes = $_GET['RealisasiRktLingkunganDungtan'];
    ...
    public function actionInputRealisasi(){
		...
		$realisasi = RealisasiRktLingkunganDungtan::model()->findByPk($post);
        $_md = RktLingkunganDungtan::model()->findByPk($realisasi->id_rkt_lingkungan_dungtan);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktLingkunganDungtan');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_lingkungan_dungtan',$realisasi->id_rkt_lingkungan_dungtan);
			$getRealisasi = RealisasiRktLingkunganDungtan::model()->find($criteria);

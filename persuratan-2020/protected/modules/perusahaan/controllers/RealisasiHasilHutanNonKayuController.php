...
//                    $tahun = $tmpbulan['1'];  
			...
			$idRkt = $rkt->id;
			$md = RktPanenHhbk::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktPanenHhbk::model()->findByAttributes(array(
						'id_rkt_panen_hhbk'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,                  //new
					));
					if(!$is_exist){
						$realisasi = new RealisasiRktPanenHhbk;
						$realisasi->id_rkt_panen_hhbk = $value->id;
            ...
            $modelPanenProduksi = new RealisasiRktPanenHhbk;
            $modelPanenProduksi->unsetAttributes();
            if (isset($_GET['RealisasiRktPanenHhbk']))
                $modelPanenProduksi->attributes = $_GET['RealisasiRktPanenHhbk'];
...
//                    die();
	...
	public function actionInputJumlahPanenProduksi(){
		...
		$realisasi = RealisasiRktPanenHhbk::model()->findByPk($post);
                $_md = RktPanenHhbk::model()->findByPk($realisasi->id_rkt_panen_hhbk);
...
//                        die();       
                ...
                $model = new TbEditableSaver('RealisasiRktPanenHhbk');
                ...
                if($name == 'realisasi_produksi'){
                        ...
                        if ((!isset($_md->luas) || $_md->luas == 0)){
                                $model->error('Rencana tidak boleh kosong');
                                die;
                        }elseif(floatval($_md->luas) < $_POST['value']){
                                $model->error('Realisasi tidak boleh lebih dari ' . $_md->luas);
                                die;
                        }
                }
                $model->update();
                ...
                if($name == 'realisasi_produksi'){        
                                if(isset($_md->jumlah) && isset($realisasi->realisasi_produksi)){
                                        $criteria = new CDbCriteria;
                                        $criteria->select = array(
                                                'COALESCE(SUM(t.realisasi_produksi), 0) AS realisasi'
                                        );
                                        $criteria->compare('t.id_rkt_panen_hhbk',$realisasi->id_rkt_panen_hhbk);
                                        $getRealisasi = RealisasiRktPanenHhbk::model()->find($criteria);
                ...
                //			$coba = number_format($coba,2);
                                        ...
                                        $realisasi->persentase_produksi = number_format((($value / $_md->jumlah) * 100), 2);
                                        $realisasi->updated = new CDbExpression('NOW()');
...
//                                        $_md->update();
                 ...
                 elseif($name == 'realisasi_luas'){        
                        if(isset($_md->luas) && isset($realisasi->realisasi_luas)){
                                $criteria = new CDbCriteria;
                                $criteria->select = array(
                                ...
                                $realisasi->persentase_luas = number_format((($value / $_md->luas) * 100), 2);
                                $realisasi->updated = new CDbExpression('NOW()');

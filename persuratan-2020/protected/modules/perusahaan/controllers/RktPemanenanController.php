...
class RktPemanenanController extends Controller {
    ...
    public function actionListRktPanenProduksi($rkt) {
        //debug($rkt);
        $model = new RktPanenProduksi;
        $model->unsetAttributes();
        $model->attributes = array(
...
//                $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('index'));
	...
	public function loadModel($id) {
        $model = RktPanenProduksi::model()->findByPk($id);
        if ($model === null)
...
//        debug($rkt); die;
        ...
        $ModelRKT = new RktPanenProduksi;

        if (isset($_POST['RktPanenProduksi'])) {
            $ModelRKT->attributes = $_POST['RktPanenProduksi'];

//             debug($_POST['RktPanenProduksi']); die;
...
//             debug($_POST['RktPanenProduksi']);            die();
             ...
             $rktRef = Rkt::model()->find(array('condition' => 'status = 1 AND id_perusahaan = ' . Yii::app()->user->idPerusahaan() . ' AND id_rku = ' . $rku->id_rku . " AND rkt_ke = " .  $_POST['RktPanenProduksi']['rkt_ke_new']));
//         debug($rktRef);            die();
            
            $model->attributes = $_POST['RktPanenProduksi'];

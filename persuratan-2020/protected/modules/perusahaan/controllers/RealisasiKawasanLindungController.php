...
class RealisasiKawasanLindungController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktKawasanLindung::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktKawasanLindung::model()->findByAttributes(array(
						'id_rkt_kawasan_lindung'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
					));
					if(!$is_exist){
						$realisasiBibit = new RealisasiRktKawasanLindung;
						$realisasiBibit->id_rkt_kawasan_lindung = $value->id;
            ...
            $modelBibit = new RealisasiRktKawasanLindung;
            $modelBibit->unsetAttributes();
            if(isset($_GET['RealisasiRktKawasanLindung']))
                $modelBibit->attributes = $_GET['RealisasiRktKawasanLindung'];
    ...
    public function actionInputJumlahRealisasi(){
		...
		$realisasi = RealisasiRktKawasanLindung::model()->findByPk($post);
        $_md = RktKawasanLindung::model()->findByPk($realisasi->id_rkt_kawasan_lindung);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktKawasanLindung');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_kawasan_lindung',$realisasi->id_rkt_kawasan_lindung);
			$getRealisasi = RealisasiRktKawasanLindung::model()->find($criteria);

...
class RealisasiPenjaranganController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
            $idRkt = $rkt->id;
			$md = RktJarang::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktJarang::model()->findByAttributes(array(
						'id_rkt_jarang'=>$value->id,
						'id_bulan'=>$id_bulan
					));
					if(!$is_exist){
						$realisasi = new RealisasiRktJarang;
						$realisasi->id_rkt_jarang = $value->id;
            ...
            $modelSulam = new RealisasiRktJarang;
            $modelSulam->unsetAttributes();
            if (isset($_GET['RealisasiRktJarang']))
                $modelSulam->attributes = $_GET['RealisasiRktJarang'];
	...
	public function actionInputJumlahSulam(){
		...
		$realisasi = RealisasiRktJarang::model()->findByPk($post);
        $_md = RktJarang::model()->findByPk($realisasi->id_rkt_jarang);

        Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktJarang');
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_jarang',$realisasi->id_rkt_jarang);
			$getRealisasi = RealisasiRktJarang::model()->find($criteria);

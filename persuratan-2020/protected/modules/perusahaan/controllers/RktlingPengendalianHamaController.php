...
class RktlingPengendalianHamaController extends Controller {
    ...
    public function accessRules() {
        return array(
            ...
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array(
                    'deletePengendalianHama',
                    'editJumlahPengendalianHama',                    
                ),
                'users' => array(Yii::app()->user->perusahaanRole()),
            ),
    ...
    public function actionIndex($rkt) {
        //debug($rkt);
        $model = new RktLingkunganDalmakit;
        $model->unsetAttributes();
        $model->attributes = array(
            'id_rkt' => $rkt
        );

        //$new_tata_batas = new RktLingkunganDalmakit;
        //$new_tata_batas->id_rkt = $rkt;
        
        if (isset($_POST['RktLingkunganDalmakit'])) {
            $model->attributes = $_POST['RktLingkunganDalmakit'];
            if ($model->save()) {
  	...
  	public function actionDeletePengendalianHama($id) {
    ...
    public function actionEditJumlahPengendalianHama() {
        Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RktLingkunganDalmakit');
        $model->update();
    }

...
class RealisasiPengadaanBibitController extends Controller
{
	public $layout=false;
    ...
    public function actionIndex()
	{
        ...
        if(isset($rkt)){
                        $idRkt = $rkt->id;
			$md = RktBibit::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktBibit::model()->findByAttributes(array(
						'id_rkt_bibit'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
					));
					if(!$is_exist){
						$realisasiBibit = new RealisasiRktBibit;
						$realisasiBibit->id_rkt_bibit = $value->id;
						$realisasiBibit->id_bulan = $id_bulan;
						$realisasiBibit->tahun = $tahun;
			...
			$cariRktBibit = RktBibit::model()->findAll(array(
				'condition' => 'id_rkt='.$rkt->id.' AND (jumlah != 0 OR jumlah != NULL)'
			));
			$all_id_rkt_bibit = "";
			foreach ($cariRktBibit as $kC => $vC) {
				$all_id_rkt_bibit .= $vC->id;
				if($kC < count($cariRktBibit)-1) $all_id_rkt_bibit .= ",";
			}
			if(empty($cariRktBibit)) $all_id_rkt_bibit = "0";
			// echo $all_id_rkt_bibit;
                        $modelBibit = new RealisasiRktBibit;
                        $modelBibit->unsetAttributes();
                
                
                if(isset($_GET['RealisasiRktBibit']))
                    $modelBibit->attributes = $_GET['RealisasiRktBibit'];
	            $modelBibit->id_rkt = $idRkt;
	            $modelBibit->id_bulan = $id_bulan;
	            $modelBibit->tahun = $tahun_periode;
    		    $modelBibit->in_id = $all_id_rkt_bibit;
	}

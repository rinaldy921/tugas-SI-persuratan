...
class RktPenanamanController extends Controller {
...
//                $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('index'));
    ...
    public function loadModel($id) {
        $model = RktTanam::model()->findByPk($id);
        if ($model === null)
    ...
    public function actionFormRktMatch($rku) {
        $ModelRKU = RkuTanam::model()->findByPk($rku);
        ...
        $ModelRKT = new RktTanam();

        if (isset($_POST['RktTanam'])) {
            $ModelRKT->attributes = $_POST['RktTanam'];
            if ($ModelRKT->save()) {
                $data = array(
                    'header' => "Sukses",
...
//         debug($ModelRKT); die;
    ...
    public function actionPilihRKU($tahun) {
        ...
        $model = new RkuTanam('searchByRkt');
...
//            $model->attributes = $_POST['RktTanam'];
             ...
             $rktRef = Rkt::model()->find(array('condition' => 'status = 1 AND id_perusahaan = ' . Yii::app()->user->idPerusahaan() . ' AND id_rku = ' . $rku->id_rku . " AND rkt_ke = " .  $_POST['RktTanam']['rkt_ke_new']));

...
class RealisasiKendaliKebakaranController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktLingkunganDalkar::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktLingkunganDalkar::model()->findByAttributes(array(
						'id_rkt_lingkungan_dalkar'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
					));
					if(!$is_exist){
						$realisasiBibit = new RealisasiRktLingkunganDalkar;
						$realisasiBibit->id_rkt_lingkungan_dalkar = $value->id;
            ...
            $modelBibit = new RealisasiRktLingkunganDalkar;
            $modelBibit->unsetAttributes();
            if(isset($_GET['RealisasiRktLingkunganDalkar']))
                $modelBibit->attributes = $_GET['RealisasiRktLingkunganDalkar'];
    ...
    public function actionInputRealisasi(){
		...
		$realisasi = RealisasiRktLingkunganDalkar::model()->findByPk($post);
        $_md = RktLingkunganDalkar::model()->findByPk($realisasi->id_rkt_lingkungan_dalkar);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktLingkunganDalkar');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_lingkungan_dalkar',$realisasi->id_rkt_lingkungan_dalkar);
			$getRealisasi = RealisasiRktLingkunganDalkar::model()->find($criteria);

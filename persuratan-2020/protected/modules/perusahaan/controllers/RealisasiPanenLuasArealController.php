...
class RealisasiPanenLuasArealController extends Controller
{
	public $layout = false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktPanenAreal::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktPanenAreal::model()->findByAttributes(array(
						'id_rkt_panen_areal'=>$value->id,
						'id_bulan'=>$id_bulan
					));
					if(!$is_exist){
						$realisasi = new RealisasiRktPanenAreal;
						$realisasi->id_rkt_panen_areal = $value->id;
            ...
            $modelTanam = new RealisasiRktPanenAreal;
            $modelTanam->unsetAttributes();
            if (isset($_GET['RealisasiRktPanenAreal']))
                $modelTanam->attributes = $_GET['RealisasiRktPanenAreal'];
	...
	public function actionInputJumlahTanam(){
		...
		$realisasi = RealisasiRktPanenAreal::model()->findByPk($post);
        $_md = RktPanenAreal::model()->findByPk($realisasi->id_rkt_panen_areal);

        Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktPanenAreal');
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_panen_areal',$realisasi->id_rkt_panen_areal);
			$getRealisasi = RealisasiRktPanenAreal::model()->find($criteria);

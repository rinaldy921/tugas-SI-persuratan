...
class RealisasiKerjasamaKoperasiController extends Controller
{
	public $layout=false;
        ...
        public function actionIndex()
	{
        ...
        if(isset($rkt)){
            $idRkt = $rkt->id;
            $md = RktKerjasamaKoperasi::model()->findAllByAttributes(array(
                    'id_rkt'=>$idRkt
            ));
                if($md){
                    foreach($md as $key => $value){
                        $is_exist = RealisasiRktKerjasamaKoperasi::model()->findByAttributes(array(
                            'id_rkt_kerjasama_koperasi'=>$value->id,
                            'id_bulan'=>$id_bulan,
                            'tahun'=>$tahun,
                        ));
                        if(!$is_exist){
                            $realisasiBibit = new RealisasiRktKerjasamaKoperasi;
                            $realisasiBibit->id_rkt_kerjasama_koperasi = $value->id;
            ...
            $modelRealisasi = new RealisasiRktKerjasamaKoperasi;
            $modelRealisasi->unsetAttributes();
                if(isset($_GET['RealisasiRktKerjasamaKoperasi']))
            $modelRealisasi->attributes = $_GET['RealisasiRktKerjasamaKoperasi'];
	...
	$realisasi = RealisasiRktKerjasamaKoperasi::model()->findByPk($post);
        $_md = RktKerjasamaKoperasi::model()->findByPk($realisasi->id_rkt_kerjasama_koperasi);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktKerjasamaKoperasi');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_kerjasama_koperasi',$realisasi->id_rkt_kerjasama_koperasi);
			$getRealisasi = RealisasiRktKerjasamaKoperasi::model()->find($criteria);

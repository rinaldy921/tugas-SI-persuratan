...
class RealisasiPenyiapanLahanController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktSiapLahan::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktSiapLahan::model()->findByAttributes(array(
						'id_rkt_siap_lahan'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,

					));
					if(!$is_exist){
						$realisasi = new RealisasiRktSiapLahan;
						$realisasi->id_rkt_siap_lahan = $value->id;
            ...
            $modelSiapLahan = new RealisasiRktSiapLahan;
            $modelSiapLahan->unsetAttributes();
            if (isset($_GET['RealisasiRktSiapLahan']))
                $modelSiapLahan->attributes = $_GET['RealisasiRktSiapLahan'];
            $modelSiapLahan->id_rkt = $rkt->id;
			$modelSiapLahan->id_bulan = $id_bulan;
			$modelSiapLahan->tahun = $tahun_periode;
		}
	...
	public function actionInputJumlahSiapLahan(){
		...
		$realisasi = RealisasiRktSiapLahan::model()->findByPk($post);
        $_md = RktSiapLahan::model()->findByPk($realisasi->id_rkt_siap_lahan);

        Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktSiapLahan');
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_siap_lahan',$realisasi->id_rkt_siap_lahan);
			$getRealisasi = RealisasiRktSiapLahan::model()->find($criteria);

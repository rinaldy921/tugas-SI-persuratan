...
class RealisasiPwhController extends Controller {
    ...
    public function actionIndex() {
        ...
        if (isset($rkt)) {
            $idRkt = $rkt->id;
            $md = RktPwh::model()->findAllByAttributes(array(
                'id_rkt' => $idRkt
            ));
            if ($md) {
                foreach ($md as $key => $value) {
                    $is_exist = RealisasiRktPwh::model()->findByAttributes(array(
                        'id_rkt_pwh' => $value->id,
                        'id_bulan' => $id_bulan,
                        'tahun'=>$tahun,
                    ));
                    if (!$is_exist) {
                        $realisasiBibit = new RealisasiRktPwh;
                        $realisasiBibit->id_rkt_pwh = $value->id;
            ...
            $modelBibit = new RealisasiRktPwh;
            $modelBibit->unsetAttributes();
            if (isset($_GET['RealisasiRktPwh']))
                $modelBibit->attributes = $_GET['RealisasiRktPwh'];
    ...
    public function actionInputJumlahRealisasi() {
        ...
        $value = $_POST['value'];
        //die("test");
        $realisasi = RealisasiRktPwh::model()->findByPk($post);
        $_md = RktPwh::model()->findByPk($realisasi->id_rkt_pwh);

        Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktPwh');
...
//            }
        ...
        if (isset($_md->jumlah) && isset($realisasi->realisasi)) {
            ...
            $criteria->compare('t.id_rkt_pwh', $realisasi->id_rkt_pwh);
            $getRealisasi = RealisasiRktPwh::model()->find($criteria);

...
class RealisasiPemeliharaanController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktPelihara::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktPelihara::model()->findByAttributes(array(
						'id_rkt_pelihara'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
					));
					if(!$is_exist){
						$realisasi = new RealisasiRktPelihara;
						$realisasi->id_rkt_pelihara = $value->id;
            ...
            $modelPelihara = new RealisasiRktPelihara;
            $modelPelihara->unsetAttributes();
            if (isset($_GET['RealisasiRktPelihara']))
                $modelPelihara->attributes = $_GET['RealisasiRktPelihara'];
            $modelPelihara->id_rkt = $rkt->id;
			$modelPelihara->id_bulan = $id_bulan;
			$modelPelihara->tahun = $tahun_periode;
		}
		$this->render('index',array(
            ...
            'modelPelihara'=>$modelPelihara
		));
	}

	public function actionInputJumlahPelihara(){
        $post = $_POST['pk'];
		$value = $_POST['value'];

		$realisasi = RealisasiRktPelihara::model()->findByPk($post);
        $_md = RktPelihara::model()->findByPk($realisasi->id_rkt_pelihara);

        Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktPelihara');
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_pelihara',$realisasi->id_rkt_pelihara);
			$getRealisasi = RealisasiRktPelihara::model()->find($criteria);

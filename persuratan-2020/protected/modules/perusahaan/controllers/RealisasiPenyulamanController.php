...
class RealisasiPenyulamanController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
            $idRkt = $rkt->id;
			$md = RktSulam::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktSulam::model()->findByAttributes(array(
						'id_rkt_sulam'=>$value->id,
						'id_bulan'=>$id_bulan
					));
					if(!$is_exist){
						$realisasi = new RealisasiRktSulam;
						$realisasi->id_rkt_sulam = $value->id;
            ...
            $modelSulam = new RealisasiRktSulam;
            $modelSulam->unsetAttributes();
            if (isset($_GET['RealisasiRktSulam']))
                $modelSulam->attributes = $_GET['RealisasiRktSulam'];
	...
	public function actionInputJumlahSulam(){
		...
		$realisasi = RealisasiRktSulam::model()->findByPk($post);
        $_md = RktSulam::model()->findByPk($realisasi->id_rkt_sulam);

        Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktSulam');
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_sulam',$realisasi->id_rkt_sulam);
			$getRealisasi = RealisasiRktSulam::model()->find($criteria);

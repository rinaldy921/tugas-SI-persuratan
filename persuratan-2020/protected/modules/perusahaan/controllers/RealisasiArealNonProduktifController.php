...
class RealisasiArealNonProduktifController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktArealNonProduktif::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktArealNonProduktif::model()->findByAttributes(array(
						'id_rkt_areal_non_produktif'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
					));
					if(!$is_exist){
						$realisasiBibit = new RealisasiRktArealNonProduktif;
						$realisasiBibit->id_rkt_areal_non_produktif = $value->id;
            ...
            $modelBibit = new RealisasiRktArealNonProduktif;
            $modelBibit->unsetAttributes();
            if(isset($_GET['RealisasiRktArealNonProduktif']))
                $modelBibit->attributes = $_GET['RealisasiRktArealNonProduktif'];
    ...
    public function actionInputJumlahRealisasi(){
		...
		$realisasi = RealisasiRktArealNonProduktif::model()->findByPk($post);
        $_md = RktArealNonProduktif::model()->findByPk($realisasi->id_rkt_areal_non_produktif);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktArealNonProduktif');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_areal_non_produktif',$realisasi->id_rkt_areal_non_produktif);
			$getRealisasi = RealisasiRktArealNonProduktif::model()->find($criteria);

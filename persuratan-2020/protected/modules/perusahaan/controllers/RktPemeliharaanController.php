...
class RktPemeliharaanController extends Controller {
...
//                $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('index'));
    ...
    public function loadModel($id) {
        $model = RktPelihara::model()->findByPk($id);
        if ($model === null)
    ...
    public function actionFormRktMatch($rku) {
        $ModelRKU = RkuPelihara::model()->findByPk($rku);
        ...
        $ModelRKT = new RktPelihara();

        if (isset($_POST['RktPelihara'])) {
            $ModelRKT->attributes = $_POST['RktPelihara'];
            ...
            $ModelRKT->attributes = array(
                ...
                '_jenis_pemeliharaan' => $ModelRKU->idJenisPemeliharaan->jenis_pemeliharaan,
                'id_rkt' => $rkt->id,
                'id_blok' => $ModelRKU->id_blok,
                'id_jenis_produksi_lahan' => $ModelRKU->id_jenis_produksi_lahan,
                'id_jenis_lahan' => $ModelRKU->id_jenis_lahan,
                'id_jenis_pemeliharaan' => $ModelRKU->id_jenis_pemeliharaan,
                'id_rku_pelihara' => $ModelRKU->id,
    ...
    public function actionPilihRKU($tahun) {
        ...
        $model = new RkuPelihara('searchByRkt');
    ...
    public function actionFormRktDismatch($alasan = 0, $tahun = 0) {
        $rku = Rku::model()->find(array('condition' => 'edit_status = 1 AND id_perusahaan = ' . Yii::app()->user->idPerusahaan()));
        $rktNew = Rkt::model()->find(array('condition' => 'status = 1 AND id_perusahaan = ' . Yii::app()->user->idPerusahaan() . ' AND id_rku = ' . $rku->id_rku . " AND rkt_ke = " . $tahun));
        ...
        if (isset($_POST['RktPelihara'])) {
            ...
            //$_POST['RktTanam']['id_rkt_tanam_alasan'] = $alasan;
             $rktRef = Rkt::model()->find(array('condition' => 'status = 1 AND id_perusahaan = ' . Yii::app()->user->idPerusahaan() . ' AND id_rku = ' . $rku->id_rku . " AND rkt_ke = " .  $_POST['RktPelihara']['rkt_ke_new']));
        
            
            $model->attributes = $_POST['RktPelihara'];
...
//            debug($model);            die();
    ...
    public function actionInputJumlahPemeliharaan() {
        $post = $_POST['pk'];
        $md = RktPelihara::model()->findByPk($post);
        Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RktPelihara');

...
class RealisasiSarprasController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktSarpras::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktSarpras::model()->findByAttributes(array(
						'id_rkt_sarpras'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
					));
					if(!$is_exist){
						$realisasiBibit = new RealisasiRktSarpras;
						$realisasiBibit->id_rkt_sarpras = $value->id;
            ...
            $modelBibit = new RealisasiRktSarpras;
            $modelBibit->unsetAttributes();
            if(isset($_GET['RealisasiRktSarpras']))
                $modelBibit->attributes = $_GET['RealisasiRktSarpras'];
    ...
    public function actionInputJumlahRealisasi(){
		...
		$realisasi = RealisasiRktSarpras::model()->findByPk($post);
        $_md = RktSarpras::model()->findByPk($realisasi->id_rkt_sarpras);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktSarpras');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_sarpras',$realisasi->id_rkt_sarpras);
			$getRealisasi = RealisasiRktSarpras::model()->find($criteria);

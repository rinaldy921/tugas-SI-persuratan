...
class RealisasiPemasaranHhbkController extends Controller {
    ...
    public function actionIndex() {
        ...
        if (isset($rkt)) {
            $idRkt = $rkt->id;
            $md = RktPasarHhbk::model()->findAllByAttributes(array(
                'id_rkt' => $idRkt
            ));
            if ($md) {
                foreach ($md as $key => $value) {
                    $is_exist = RealisasiRktPasarHhbk::model()->findByAttributes(array(
                        'id_rkt_pasar_hhbk' => $value->id,
                        'id_bulan' => $id_bulan,
                        'tahun'=>$tahun,
                    ));
                    if (!$is_exist) {
                        $realisasi = new RealisasiRktPasarHhbk;
                        $realisasi->id_rkt_pasar_hhbk = $value->id;
            ...
            $modelPasarHhbk = new RealisasiRktPasarHhbk;
            $modelPasarHhbk->unsetAttributes();
            if (isset($_GET['RealisasiRktPasarHhbk']))
            $modelPasarHhbk->attributes = $_GET['RealisasiRktPasarHhbk'];
            $modelPasarHhbk->id_rkt = $rkt->id;
            $modelPasarHhbk->id_bulan = $id_bulan;
            $modelPasarHhbk->tahun = $tahun_periode;
//                    echo '<pre>';
//                    print_r($ukuran_file);
//                    echo '</pre>';
        }
    ...
    public function actionInputRealisasi() {
        ...
        $realisasi = RealisasiRktPasarHhbk::model()->findByPk($post);
        $_md = RktPasarHhbk::model()->findByPk($realisasi->id_rkt_pasar_hhbk);

        Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktPasarHhbk');
        ...
        if (isset($_md->jumlah) && isset($realisasi->realisasi)) {
            ...
            $criteria->compare('t.id_rkt_pasar_hhbk', $realisasi->id_rkt_pasar_hhbk);
            $getRealisasi = RealisasiRktPasarHhbk::model()->find($criteria);

...
class RealisasiMasukGunaAlatController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktMasukGunaAlat::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktMasukGunaAlat::model()->findByAttributes(array(
						'id_rkt_masuk_guna_alat'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
					));
					if(!$is_exist){
						$realisasiBibit = new RealisasiRktMasukGunaAlat;
						$realisasiBibit->id_rkt_masuk_guna_alat = $value->id;
            ...
            $modelBibit = new RealisasiRktMasukGunaAlat;
            $modelBibit->unsetAttributes();
            if(isset($_GET['RealisasiRktMasukGunaAlat']))
                $modelBibit->attributes = $_GET['RealisasiRktMasukGunaAlat'];
    ...
    public function actionInputJumlahRealisasi(){
		...
		$realisasi = RealisasiRktMasukGunaAlat::model()->findByPk($post);
        $_md = RktMasukGunaAlat::model()->findByPk($realisasi->id_rkt_masuk_guna_alat);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktMasukGunaAlat');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_masuk_guna_alat',$realisasi->id_rkt_masuk_guna_alat);
			$getRealisasi = RealisasiRktMasukGunaAlat::model()->find($criteria);

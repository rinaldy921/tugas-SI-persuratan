...
class RktprasArealTidakEfektifController extends Controller {
        ...
        return array(
            ...
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array(
                    ...
                    'editJumlahKawasanLindung',
                    'daftarKawasanLindung',
                ),
                'users' => array(Yii::app()->user->perusahaanRole()),
            ),
    ...
    public function actionDaftarKawasanLindung($rkt) {
        // debug($rkt);
        $cek = RktArealNonProduktif::model()->findByAttributes(array('id_rkt' => $rkt));
        if(is_null($cek)) {
            $mdl = new RktArealNonProduktif;
            $mdl->id_rkt = $rkt;
            $mdl->save(false);
        }

        $model = new RktArealNonProduktif;
        $model->unsetAttributes();
        $model->attributes = array(
            'id_rkt' => $rkt
        );

        // if (isset($_POST['RktArealNonProduktif'])) {
        //     $model->attributes = $_POST['RktArealNonProduktif'];
        //     if ($model->save()) {
    ...
    public function actionEditJumlahKawasanLindung() {

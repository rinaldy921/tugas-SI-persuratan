...
class RealisasiBangunMitraController extends Controller
{
	public $layout=false;
        ...
        public function actionIndex()
	{
        ...
        if(isset($rkt)){
            $idRkt = $rkt->id;
            $md = RktBangunMitra::model()->findAllByAttributes(array(
                    'id_rkt'=>$idRkt
            ));
                if($md){
                    foreach($md as $key => $value){
                        $is_exist = RealisasiRktBangunMitra::model()->findByAttributes(array(
                            'id_rkt_bangun_mitra'=>$value->id,
                            'id_bulan'=>$id_bulan,
                            'tahun'=>$tahun,
                        ));
                        if(!$is_exist){
                            $realisasiBibit = new RealisasiRktBangunMitra;
                            $realisasiBibit->id_rkt_bangun_mitra = $value->id;
            ...
            $modelRealisasi = new RealisasiRktBangunMitra;
            $modelRealisasi->unsetAttributes();
                if(isset($_GET['RealisasiRktBangunMitra']))
            $modelRealisasi->attributes = $_GET['RealisasiRktBangunMitra'];
	...
	$realisasi = RealisasiRktBangunMitra::model()->findByPk($post);
        $_md = RktBangunMitra::model()->findByPk($realisasi->id_rkt_bangun_mitra);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktBangunMitra');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_bangun_mitra',$realisasi->id_rkt_bangun_mitra);
			$getRealisasi = RealisasiRktBangunMitra::model()->find($criteria);

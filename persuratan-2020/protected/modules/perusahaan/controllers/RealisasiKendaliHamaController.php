...
class RealisasiKendaliHamaController extends Controller
{
	public $layout=false;
	...
	public function actionIndex()
	{
        ...
        if(isset($rkt)){
			$idRkt = $rkt->id;
			$md = RktLingkunganDalmakit::model()->findAllByAttributes(array(
				'id_rkt'=>$idRkt
			));
			if($md){
				foreach($md as $key => $value){
					$is_exist = RealisasiRktLingkunganDalmakit::model()->findByAttributes(array(
						'id_rkt_lingkungan_dalmakit'=>$value->id,
						'id_bulan'=>$id_bulan,
                                                'tahun'=>$tahun,
					));
					if(!$is_exist){
						$realisasiBibit = new RealisasiRktLingkunganDalmakit;
						$realisasiBibit->id_rkt_lingkungan_dalmakit = $value->id;
            ...
            $modelRealisasi = new RealisasiRktLingkunganDalmakit;
            $modelRealisasi->unsetAttributes();
            if(isset($_GET['RealisasiRktLingkunganDalmakit']))
            $modelRealisasi->attributes = $_GET['RealisasiRktLingkunganDalmakit'];
            $modelRealisasi->id_rkt = $idRkt;
            $modelRealisasi->id_bulan = $id_bulan;
            $modelRealisasi->tahun = $tahun_periode;
		}
    ...
    public function actionInputRealisasi(){
		...
		$realisasi = RealisasiRktLingkunganDalmakit::model()->findByPk($post);
        $_md = RktLingkunganDalmakit::model()->findByPk($realisasi->id_rkt_lingkungan_dalmakit);

		Yii::import('booster.components.TbEditableSaver');
        $model = new TbEditableSaver('RealisasiRktLingkunganDalmakit');
...
//                        }
		...
		if(isset($_md->jumlah) && isset($realisasi->realisasi)){
			...
			$criteria->compare('t.id_rkt_lingkungan_dalmakit',$realisasi->id_rkt_lingkungan_dalmakit);
			$getRealisasi = RealisasiRktLingkunganDalmakit::model()->find($criteria);

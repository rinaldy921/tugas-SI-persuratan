...
<script type="text/javascript">
    ...
    {
        var form = new FormData($("#ArealKerja-form")[0]);
        $.ajax({
            type: "POST",
            data: form,
            dataType: "json",
            contentType: false,
            processData: false,
            url: "<?= Yii::app()->createUrl('/perusahaan/rkuPersyaratan/addArealKerja', array('id_rku' => $id_rku)) ?>id_areal_kerja/<?= $id_areal_kerja ?>",
                        success: function (result) {
                            swal(result.header, result.message, result.status).then((ok) => {
                                if (result.status == "success") {
                                    $.fn.yiiGridView.update("rku-areal-kerja-grid");
                                    $("#modal").modal("hide");
                                }
                            });
                        },

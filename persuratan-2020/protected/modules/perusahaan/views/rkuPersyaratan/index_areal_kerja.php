...
//                            'htmlOptions' => array('confirm' => Yii::t('app', 'Form yang telah diisi akan hilang, lanjutkan pembatalan?'), 'class' => 'basebottom', 'onclick' => "window.location.href = '" . CHtml::normalizeUrl(array('index')) . "'"),
 ...
 <script type="text/javascript">
  ...
  <script  type="text/javascript"> 
    ...
    function editArealKerja(th = '')
    {
       var urlLink=$(th).attr("data-url");
       var url = urlLink;
       var title = "Ubah Data Penataan Areal Kerja";
       showModal(url, title);
       jQuery('#rku-areal-kerja-grid').yiiGridView('update');
    }
</script>
...
//            'footer' => RkuKawasanLindung::model()->getTotal($model->search()->getData(), 'jumlah') . ' Ha',
        ...
        array(
                ...
                'value'=>function($data){
					...
					'onclick'=>'editArealKerja(this)', 
                                        'data-url'=> Yii::app()->createUrl("/perusahaan/rkuPersyaratan/edit",array("id_rku"=>'','id_areal_kerja'=>$data->id)),
                            ));
			},

...
<?php echo $form->textFieldGroup($model, '_jenis_pemasaran', array('widgetOptions' => array('htmlOptions' => array('class' => 'span5', 'maxlength' => 50, 'readonly' => true)))); ?>
...
<?php echo CHtml::activeHiddenField($model, 'id_pemasaran'); ?>
...
<script type="text/javascript">
    function simpanRKT()
    {
        ...
        $.ajax({
            ...
            contentType: false,
            processData: false,
            url: "<?= Yii::app()->createUrl("/perusahaan/rktPemasaran/formRktMatch/rku/".$modelRKU->id); ?>",
            success: function(result) {
                swal(result.header, result.message, result.status).then((ok) => {
                    if(result.status == "success") {
                        $.fn.yiiGridView.update("rkt-pemasaran-grid");
                        $("#modal").modal("hide");

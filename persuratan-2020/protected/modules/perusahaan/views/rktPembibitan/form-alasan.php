...
<script type="text/javascript">
    ...
    function simpanAlasan()
    {
        ...
        $.ajax({
            ...
            contentType: false,
            processData: false,
            url: "<?= Yii::app()->createUrl("/perusahaan/rktPembibitan/entryAlasan/tahun/" . $tahun) ?>",
            success: function (result) {
                swal(result.header, result.message, result.status).then((ok) => {
                    if (result.status == "success") {
                        // $.fn.yiiGridView.update("rktBibit-tanam-grid");
                        // $("#modal").modal("hide");
                        var url = "<?php echo $this->createUrl('//perusahaan/rktPembibitan/formRktDismatch/'); ?>" + "alasan/" + result.id + "/tahun/" + result.rkt_ke;

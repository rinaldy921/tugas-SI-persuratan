...
$select_jenis_pemeliharaan = MasterJenisPemeliharaan::model()->findAll();
$list_pemeliharaan = CHtml::listData($select_jenis_pemeliharaan, 'id', function($model) {
            $gabung = $model->jenis_pemeliharaan;
            return $gabung;
        });
        
$form = $this->beginWidget('booster.widgets.TbActiveForm', array(
    'id' => 'PemeliharaanRKU-form',
    'type' => 'horizontal',
...
<?php echo $form->textFieldGroup($model, 'daur', array('widgetOptions' => array('htmlOptions' => array('class' => 'span5', 'maxlength' => 255)))); ?>
 ...
 <div class="form-group">    
        ...
        <div class="col-sm-9">
        <?php   $rkuId = Yii::app()->session['rkt_id'];
...
<?php echo $form->select2Group($model, 'id_jenis_pemeliharaan', array('groupOptions' => array('id' => "id_jenis_pemeliharaan"), 'widgetOptions' => array('options' => array('allowClear' => true), 'data' => $list_pemeliharaan, 'htmlOptions' => array('class' => 'form-control ', 'placeholder' => Yii::t('app', 'Pilih Jenis Pemeliharaan...'))))); ?>
...
<script type="text/javascript">
        $("#RkuPelihara_tahun_ke").change(function(){

            //var dataToPost = "{namaBlok:'" + $("#Rku_blok").val() + "',idSektor:" +$("#Rku_sektor option:selected").val()+ "}";
            tahun = $("#RkuPelihara_tahun_ke option:selected").val();
            
            $("#RkuPelihara_tahun").val(tahun);

            $("#RkuPelihara_rkt_ke").val($("#RkuPelihara_tahun_ke option:selected").html());
       });
...
<script type="text/javascript">
    ...
    {
        var form = new FormData($("#PemeliharaanRKU-form")[0]);
        $.ajax({
            type: "POST",
            data: form,
            dataType: "json",
            contentType: false,
            processData: false,
            url: "<?= Yii::app()->createUrl('/perusahaan/rkuKelestarian/addPemeliharaan', array('id_rku' => $id_rku)) ?>id_pemeliharaan/<?= $id_pemeliharaan ?>",

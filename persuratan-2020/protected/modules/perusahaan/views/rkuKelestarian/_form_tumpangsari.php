...
<script type="text/javascript">
        $("#RkuTanam_tahun_ke").change(function(){

            //var dataToPost = "{namaBlok:'" + $("#Rku_blok").val() + "',idSektor:" +$("#Rku_sektor option:selected").val()+ "}";
            tahun = $("#RkuTanam_tahun_ke option:selected").val();
            
            $("#RkuTanam_tahun").val(tahun);
            $("#RkuTanam_rkt_ke").val($("#RkuTanam_tahun_ke option:selected").html());
...
<script type="text/javascript">
    ...
    {
        var form = new FormData($("#tanamRKU-form")[0]);
        $.ajax({
            type: "POST",
            data: form,
            dataType: "json",
            contentType: false,
            processData: false,
            url: "<?= Yii::app()->createUrl('/perusahaan/rkuKelestarian/addPenanaman', array('id_rku' => $id_rku)) ?>id_tanam/<?= $id_tanam ?>",

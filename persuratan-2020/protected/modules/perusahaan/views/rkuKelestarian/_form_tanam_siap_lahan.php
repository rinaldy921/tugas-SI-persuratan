...
<?php echo $form->textFieldGroup($model, 'tahun', array('widgetOptions' => array('htmlOptions' => array('class' => 'span5', 'maxlength' => 255)))); ?>
    ...
    <?php echo $form->select2Group($model, 'id_jenis_produksi_lahan', array('groupOptions' => array('id' => "jenis_produksi"), 'widgetOptions' => array('options' => array('allowClear' => true), 'data' => $list_rkusil, 'htmlOptions' => array('class' => 'form-control ', 'placeholder' => Yii::t('app', 'Pilih Tanaman Silvikultur...'))))); ?>
...
<script type="text/javascript">
        $("#RkuPenyiapanLahan_tahun_ke").change(function(){

            //var dataToPost = "{namaBlok:'" + $("#Rku_blok").val() + "',idSektor:" +$("#Rku_sektor option:selected").val()+ "}";
            tahun = $("#RkuPenyiapanLahan_tahun_ke option:selected").val();
            
            $("#RkuPenyiapanLahan_tahun").val(tahun);

             $("#RkuPenyiapanLahan_rkt_ke").val($("#RkuPenyiapanLahan_tahun_ke option:selected").html());
       });
...
<script type="text/javascript">
    ...
    {
        var form = new FormData($("#siaplahanRKU-form")[0]);
        $.ajax({
            type: "POST",
            data: form,
            dataType: "json",
            contentType: false,
            processData: false,
            url: "<?=Yii::app()->createUrl('/perusahaan/rkuKelestarian/addSiapLahan',array('id_rku'=>$id_rku))?>id_siaplahan/<?=$id_siaplahan?>",
            success: function(result) {
                swal(result.header, result.message, result.status).then((ok) => {
                    if(result.status == "success") {
                        $.fn.yiiGridView.update("rkuKelestarian-siaplahan-grid");
                        $("#modal").modal("hide");
                    }
                });
            },

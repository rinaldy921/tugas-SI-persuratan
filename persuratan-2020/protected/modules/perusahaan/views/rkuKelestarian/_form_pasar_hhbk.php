...
$select_jpasar = MasterJenisPemasaran::model()->findAll();
$list_jpasar = CHtml::listData($select_jpasar, 'id', function($model) {
            $gabungpasar = $model->nama_pemasaran;
            return $gabungpasar;
        });

$form=$this->beginWidget('booster.widgets.TbActiveForm',array(
    'id'=> 'pasar_hhbk_rku-form',
        'type'=>'horizontal',
...
<div class="form-group">    
    ...
    <?php echo $form->select2Group($model, 'id_jenis_pasar', array('groupOptions' => array('id' => "id_jenis_pasar"), 'widgetOptions' => array('options' => array('allowClear' => true), 'data' => $list_jpasar, 'htmlOptions' => array('class' => 'form-control ', 'placeholder' => Yii::t('app', 'Pilih Jenis Pemasaran...'))))); ?>

    <?php echo $form->textFieldGroup($model,'jumlah',array('widgetOptions'=>array('htmlOptions'=>array('class'=>'span5','maxlength'=>255)))); ?>
    <button type="button" name="button" class="btn btn-primary pull-right" onclick="simpanPasarHHBK()">Simpan</button>

<?php $this->endWidget(); ?>

    
    <script type="text/javascript">
        $("#RkuPasarHhbk_tahun_ke").change(function(){

            //var dataToPost = "{namaBlok:'" + $("#Rku_blok").val() + "',idSektor:" +$("#Rku_sektor option:selected").val()+ "}";
            tahun = $("#RkuPasarHhbk_tahun_ke option:selected").val();
            
            $("#RkuPasarHhbk_tahun").val(tahun);

            

             $("#RkuPasarHhbk_rkt_ke").val($("#RkuPasarHhbk_tahun_ke option:selected").html());
       });
...
<script type="text/javascript">
    ...
    function simpanPasarHHBK()
    {
        var form = new FormData($("#pasar_hhbk_rku-form")[0]);
        $.ajax({
            type: "POST",
            data: form,
            dataType: "json",
            contentType: false,
            processData: false,
            url: "<?=Yii::app()->createUrl("/perusahaan/rkuKelestarian/addPasarHhbk",array('id_rku'=>$id_rku))?>?id_pasar_hhbk=<?=$id_pasar_hhbk?>",

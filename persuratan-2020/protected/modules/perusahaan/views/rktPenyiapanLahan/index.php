...
$this->widget('booster.widgets.BootGroupGridView', array(
    'id' => 'rkt-penyiapan-lahan-grid-rku-match',
...
//          'value' => '!empty($data->jumlah) ? number_format($data->jumlah,2,",",".") : ""',
            'editable' => array('url' => $this->createUrl('//perusahaan/rktPenyiapanLahan/inputJumlahLahan'),
                'success' => 'js:function() {
	                $.fn.yiiGridView.update("rkt-penyiapan-lahan-grid-rku-match",{data:"aksi=updateGrid&tahun=' . $tahun . '"});
	            }'
            ),
            'htmlOptions' => array('style' => 'text-align:right'),
            'footer' => '<strong>'.number_format(RktSiapLahan::model()->getTotal($model->findRkuMatch()->getData(), 'jumlah'),2,",",".") . '</strong>',
        ...
        array(
            ...
            'value' => function($data) {
                return CHtml::link('<i class="glyphicon glyphicon-trash"></i>', '', array(
                            ...
                            'data-url' => Yii::app()->createUrl("/perusahaan/rktPenyiapanLahan/delete", array("id" => $data->id))
                ));
            }
        ),
    ),
...
<?php
$this->widget('booster.widgets.BootGroupGridView', array(
    'id' => 'rkt-penyiapan-lahan-grid-rku-dismatch',
    'type' => 'bordered condensed striped',
    'responsiveTable' => true,
...
//          'value' => '!empty($data->jumlah) ? number_format($data->jumlah,2,",",".") : ""',
            ...
            'htmlOptions' => array('style' => 'text-align:right'),
            'footer' => '<strong>'.number_format(RktSiapLahan::model()->getTotal($model->findRkuDismatch()->getData(), 'jumlah'),2,",",".") . '</strong>',
            'footerHtmlOptions' => array('style' => 'text-align:right; vertical-align:middle'),
        ),
        array(
            'header' => 'Keterangan',
        ...
        array(
            ...
            'value' => function($data) {
                return CHtml::link('<i class="glyphicon glyphicon-eye-open"></i>', '', array(
                            ...
                            'data-url' => Yii::app()->createUrl("/perusahaan/rktBibit/showAlasan", array("id" => $data->id_rkt_form_alasan, 'model' => 'RktSiapLahan'))
                ));
            }
        ),
    ),
...
<script type="text/javascript">
    ...
    function deleteDataMatch(th) {
        ...
        if (confirm("Apakah anda yakin ingin menghapus item ini ?") == true) {
            ...
            jQuery('#rkt-penyiapan-lahan-grid-rku-match').yiiGridView('update', {
                type: 'POST',
                url: urlLink,
    ...
    function deleteDataDisMatch(th) {
        ...
        if (confirm("Apakah anda yakin ingin menghapus item ini ?") == true) {
            ...
            //var th = this,
            afterDelete = function () {};
            jQuery('#rkt-penyiapan-lahan-grid-rku-dismatch').yiiGridView('update', {
                type: 'POST',
                url: urlLink,
                success: function (data) {
                    jQuery('#rkt-penyiapan-lahan-grid-rku-dismatch').yiiGridView('update');
    ...
    function show_form_alasan()
    {
        ...
        var url = "<?php echo $this->createUrl('//perusahaan/rktPenyiapanLahan/entryAlasan'); ?>" + "tahun/" + thn;
    ...
    function entry_rkt_by_rku()
    {
        var thn = $("#Rkt_tahun_mulai").val();
        //alert(thn);
        var url = "<?php echo $this->createUrl('//perusahaan/rktPenyiapanLahan/pilihRKU'); ?>" + "tahun/" + thn;

...
<?php// echo $form->textFieldGroup($model, 'daur', array('widgetOptions' => array('htmlOptions' => array('class' => 'span5', 'maxlength' => 50)))); ?>
...
<?php echo $form->select2Group($model, 'id_jenis_tanaman', array('groupOptions' => array('id' => "nama_tanaman"), 'widgetOptions' => array('options' => array('allowClear' => true), 'data' => $listTanaman, 'htmlOptions' => array('class' => 'form-control ', 'placeholder' => Yii::t('app', 'Pilih Jenis Tanaman'))))); ?>

<?php echo $form->textFieldGroup($model, 'jumlah', array('widgetOptions' => array('htmlOptions' => array('class' => 'span5', 'maxlength' => 50)))); ?>

<?php echo CHtml::activeHiddenField($model, 'id_rku_tanam'); ?>
...
<div class="form-group">
    ...
    <div class="col-sm-9">
        ...
        $this->widget('booster.widgets.TbButton', array(
            ...
            'htmlOptions' => array('onclick' => 'toggleFormRKTByRKU("#form-rkt-tanam","#select-rku-tanam","'.$this->createUrl('//perusahaan/inputRKT/formRKTTanamByRKU/tahun/'.$modelRKU->tahun).'")'),
        ));
        ?>
    </div>
</div>
...
<script type="text/javascript">
    function simpanRKT()
    {
        ...
        $.ajax({
            ...
            contentType: false,
            processData: false,
            url: "<?= Yii::app()->createUrl("/perusahaan/inputRKT/formRKTTanam/rkutanam/".$modelRKU->id); ?>",
            success: function(result) {
                swal(result.header, result.message, result.status).then((ok) => {
                    if(result.status == "success") {
                        $.fn.yiiGridView.update("rkt-penanaman-grid");
                        $("#modal").modal("hide");

<style media="screen">
    #<?php echo Yii::app()->controller->id ?>-siaplahan-grid table th{
        text-align: center;
...
<script type="text/javascript">
    function generateTable(){
        ...
        temp += '</tr>';
        $('#<?=Yii::app()->controller->id?>-siaplahan-grid table thead').prepend(temp);
    }
    generateTable();
</script>
...
<center><h3>Realisasi Penyiapan Lahan</h3></center> 
...
<?php 
 if($status_laporan == 0){  
                ...
                'enableSorting'=>false,
                'dataProvider'=>$modelSiapLahan->searchByRkt(),
                ...
                'extraRowTotals'=>function($data, $row, &$totals) {
                    ...
                    $totals['jumlah'] += $data['idRktSiapLahan']['jumlah'];
                ...
                'columns'=>array(
                        array(
                            'header' => 'Daur',
                            'name' => 'daur',
                            'value'=>'$data->idRktSiapLahan->daur',
                            'headerHtmlOptions' => array('class' => 'hidden')
                        ),
                        array(
                            'header' => 'Sektor',
                            'name' => 'id_sektor',
                            'value'=>'$data->idRktSiapLahan->idBlok->namaSektor->nama_sektor',
                            'headerHtmlOptions' => array('class' => 'hidden'),
                            'footer' => '<strong>Total</strong>'
                        ),
                        array(
                            'name' => 'id_blok',
                            'value'=>'$data->idRktSiapLahan->idBlok->nama_blok',
                            'headerHtmlOptions' => array('class' => 'hidden')
                         ),
                        array(
                            'header'=>'Produksi Lahan',
                            'value'=>'$data->idRktSiapLahan->idJenisProduksiLahan->jenis_produksi',
                            'headerHtmlOptions' => array('class' => 'hidden')
                        ),
                        array(
                            'header'=>'Jenis Lahan',
                            'name'=>'id_jenis_lahan',
                            'headerHtmlOptions' => array('class' => 'hidden'),
                            'value'=>'$data->idRktSiapLahan->idJenisLahan->jenis_lahan',
                            ),
                        array(
                            'header'=>'Rencana',
                            'headerHtmlOptions' => array('class' => 'hidden'),
                            'value'=>'isset($data->idRktSiapLahan->jumlah) ? number_format($data->idRktSiapLahan->jumlah,2,",",".") : "0"',
                            'footer' => '<strong>'.$modelSiapLahan->getTotal($modelSiapLahan->searchByRkt()->getData(), 'jumlah').'</strong>',            
                        ),
                        array(
                            'header'=>'sd Bulan Lalu',
                            'value'=>'$data->sd_bulan_lalu',
                            'footer' => '<strong>'.$modelSiapLahan->getTotal($modelSiapLahan->searchByRkt()->getData(), 'sd_bulan_lalu').'</strong>'
                        ),
                        array(
                            'header'=>'Bulan Ini',
                            ...
                            'editable'=> array('url'=>$this->createUrl('//perusahaan/realisasiPenyiapanLahan/inputJumlahSiapLahan'),
            //    				'success'=>'js:function(){
            //                        $.fn.yiiGridView.update("'.Yii::app()->controller->id.'-siaplahan-grid",{data:"aksi=updateGrid&tahun='. $tahun .'&id_bulan='. $id_bulan .'&tahun_periode='. $tahun_periode .'",complete:function(){
            //                            setTimeout(function(){
            //                                generateTable();
            //                            }, 100)
            //                        }});
            ...
            //                    }'
                            ...
                            'footer' => '<strong>'.$modelSiapLahan->getTotal($modelSiapLahan->searchByRkt()->getData(), 'realisasi').'</strong>',
                        ),
                        array(
                            'header'=>'sd Bulan Ini',
                            'value'=>'$data->sd_sekarang',
                            'footer' => '<strong>'.$modelSiapLahan->getTotal($modelSiapLahan->searchByRkt()->getData(), 'sd_sekarang').'</strong>'
                        ),
                            array(
                            'name'=>'persentase',
                            'headerHtmlOptions' => array('class' => 'hidden'),
                            'value'=>'isset($data->persentase) ? number_format($data->persentase,2,",",".") : "0"',
                            'footer' => '<strong>'.$modelSiapLahan->getTotalPersen($modelSiapLahan->searchByRkt()->getData(), 'persentase').'</strong>',
                        )
                ),
            )); 
 }else{

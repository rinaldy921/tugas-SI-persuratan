...
//                ),
            ...
            'footer' => '<strong>'.number_format(RktPanenProduksi::model()->getTotal($model->findRkuMatch()->getData(), 'jumlah_luas'),2,",",".") . '</strong>',
...
//                ),
            'htmlOptions' => array('style' => 'text-align:right'),
            'footer' => '<strong>'.number_format(RktPanenProduksi::model()->getTotal($model->findRkuMatch()->getData(), 'jumlah_produksi'),2,",",".") . '</strong>',
            'footerHtmlOptions' => array('style' => 'text-align:right; vertical-align:middle'),
        ),
        array(
            'header'=>'',
...
//                ),
            'htmlOptions' => array('style' => 'text-align:right'),
            'footer' => '<strong>'.number_format(RktPanenProduksi::model()->getTotal($model->findRkuDismatch()->getData(), 'jumlah_luas'),2,",",".") . '</strong>',
            'footerHtmlOptions' => array('style' => 'text-align:right; vertical-align:middle'),
        ),
        array(
            'header'=>'Jumlah (m³)',
...
//                ),
            'htmlOptions' => array('style' => 'text-align:right'),
            'footer' => '<strong>'.number_format(RktPanenProduksi::model()->getTotal($model->findRkuDismatch()->getData(), 'jumlah_produksi'),2,",",".") . '</strong>',
            'footerHtmlOptions' => array('style' => 'text-align:right; vertical-align:middle'),
        ),
        array(
            'header' => 'Keterangan',
...
<script type="text/javascript">
	...
	function deleteDataDisMatch(th){
		...
		if (confirm("Apakah anda yakin ingin menghapus item ini ?") == true) {
			...
			//var th = this,
			afterDelete = function(){};
			jQuery('#rkt-pembibitan-grid-rku-dismatch').yiiGridView('update', {
				type: 'POST',
				url: urlLink,
				success: function(data) {
					jQuery('#rkt-pembibitan-grid-rku-dismatch').yiiGridView('update');
    ...
    function show_form_alasan()
    {
        ...
        var url = "<?php echo $this->createUrl('//perusahaan/rktPemanenan/entryAlasan'); ?>" + "tahun/" + thn;
    ...
    function entry_rkt_by_rku()
    {
        var thn = $("#Rkt_tahun_mulai").val();
        //alert(thn);
        var url = "<?php echo $this->createUrl('//perusahaan/rktPemanenan/pilihRKU'); ?>" + "tahun/" + thn;

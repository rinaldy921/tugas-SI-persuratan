...
<div class="form-group">
    ...
    <div class="col-sm-9">
        ...
        $this->widget('booster.widgets.TbButton', array(
            ...
            'htmlOptions' => array('onclick' => 'toggleFormRKTByRKU("#form-rkt","#select-rku","'.$this->createUrl('//perusahaan/rktPenanaman/pilihRKU/tahun/'.$modelRKU->tahun).'")'),
        ));
        ?>
    </div>
</div>
...
<script type="text/javascript">
    function simpanRKT()
    {
        ...
        $.ajax({
            ...
            contentType: false,
            processData: false,
            url: "<?= Yii::app()->createUrl("/perusahaan/rktPenanaman/formRktMatch/rku/".$modelRKU->id); ?>",
            success: function(result) {
                swal(result.header, result.message, result.status).then((ok) => {
                    if(result.status == "success") {
                        $.fn.yiiGridView.update("rkt-penanaman-grid-rku-match");
                        $("#modal").modal("hide");

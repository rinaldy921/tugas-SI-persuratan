...
//            'value' => '!empty($data->jumlah) ? number_format($data->jumlah,0,",",".") : ""',
            'editable' => array(
                'url' => $this->createUrl('//perusahaan/rktPenanaman/inputJumlahTanam'),
                'success' => 'js:function() {
	                $.fn.yiiGridView.update("rkt-penanaman-grid-rku-match",{data:"aksi=updateGrid&tahun=' . $tahun . '"});
	            }'
                ),
            'htmlOptions' => array('style' => 'text-align:right'),
            'footer' => '<strong>'.number_format(RktTanam::model()->getTotal($model->findRkuMatch()->getData(), 'jumlah'),2,",",".") . '</strong>',
...
//          'value' => '!empty($data->jumlah) ? number_format($data->jumlah,2,",",".") : ""',
            ...
            'htmlOptions' => array('style' => 'text-align:right'),
            'footer' => '<strong>'.number_format(RktTanam::model()->getTotal($model->findRkuDismatch()->getData(), 'jumlah'),2,",",".") . '</strong>',
            'footerHtmlOptions' => array('style' => 'text-align:right; vertical-align:middle'),
        ),
        array(
            'header' => 'Keterangan',
        ...
        array(
            ...
            'value'=>function($data){
                    return CHtml::link('<i class="glyphicon glyphicon-eye-open"></i>','',array(
                            ...
                            'data-url'=> Yii::app()->createUrl("/perusahaan/rktBibit/showAlasan",array("id"=>$data->id_rkt_form_alasan,'model'=>'RktTanam'))
                    ));
            }
        ),
    ),

...
<script type="text/javascript">
    ...
    function simpanRKT()
    {
        ...
        $('#RktArealKerja_rkt_ke_new').val($('#Rkt_tahun_mulai').val());
        ...
        $.ajax({
            ...
            contentType: false,
            processData: false,
            url: "<?= Yii::app()->createUrl("/perusahaan/rktPenataanAreal/formRktDismatch/alasan/" . $alasan . "/tahun/" . $tahun) ?>",
            success: function (result) {
                swal(result.header, result.message, result.status).then((ok) => {
                    if (result.status == "success") {
                        $.fn.yiiGridView.update("rkt-pak-grid-rku-dismatch");
                        $("#modal").modal("hide");

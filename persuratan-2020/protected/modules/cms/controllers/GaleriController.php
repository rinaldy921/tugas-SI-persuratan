...
class GaleriController extends Controller {
    ...
    protected function getIndexList($model) {
        $indexList = array(
            ...
            array(
                ...
                'name' => 'Deskripsi',
                'type' => 'raw',
                'value' => '($data->Deskripsi) ? $data->wp_trim(strip_tags($data->Deskripsi), 60) : "-"',
            ), array(
                'name' => 'Cover',
                'type' => 'raw',
    ...
    protected function getViewList($model) {
        $viewList = array(
            array(
                ...
                'name' => 'Lampiran',
                'label' => Yii::t('view', 'Berkas/Foto'),
                'type' => 'raw',
                'value' => !empty($model->attachments) ? nl2br($model->listAttachment($model->attachments, 'album')) : '-',
            ), array(
                'name' => 'created_at',
    ...
    public function init() {
        parent::init();
        if(Yii::app()->user->checkAccess('Cms.Galeri.Admin')) {
            $this->defaultAction = 'admin';
        }
    }

    public function actionRead($slug) {
        $model = Publikasi::model()->Galeri()->find("slug = '$slug'");
            ...
            if ($model->save()) {
                ...
                if (isset($photos[0])) {
                    foreach ($photos as $image => $pic) {
                        if ($pic->saveAs($target . $pic->name)) {
                            $img_add = new Attachment;
                            $img_add->File_Name = $pic->name;
                            $img_add->File_Path = Yii::app()->params->uploadDir . 'images/' . $pic->name;
                            $img_add->File_Size = $pic->size;
                            $img_add->File_Type = $pic->type;
                            $img_add->Keterangan = $model->Judul;
                            $img_add->publikasi_id = $model->id;
                            $img_add->save();
                            $image = Yii::app()->image->load($target . $pic->name);
                            $image->resize(171, 180)->quality(60)->sharpen(20);
                            $image->save($target . 'thumb' . DIRECTORY_SEPARATOR . $pic->name);
                        } else {
                            echo 'Cannot upload!';
                        }
                    }
            ...
            if ($model->save()) {
                ...
                if (isset($_POST['del_file']) && !empty($_POST['del_file'])) {
                    foreach ($_POST['del_file'] as $df) {
                        ...
                        if ($att_->delete()) {
                            @unlink($target . $df_name);
                            @unlink($target . 'thumb' . DIRECTORY_SEPARATOR . $df_name);
                        }
                    }
                }
                $photos = CUploadedFile::getInstancesByName('File_Name');

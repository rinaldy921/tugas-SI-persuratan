    ...
    public function accessRules() {
        return array(
            ...
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('create', 'update','getKabupaten'),
                'users' => array(Yii::app()->user->adminRole()),
            ),
...
// $this->performAjaxValidation($model);
        ...
        if (isset($_POST['Perusahaan'])) {
            ...
            $model->provinsi = !empty($_POST['Perusahaan']['provinsi']) ? $_POST['Perusahaan']['provinsi'] : null;
            $model->kabupaten = !empty($_POST['Perusahaan']['provinsi']) && !empty($model->provinsi_id) ? $_POST['Perusahaan']['kabupaten'] : null;
            if ($model->save()) {
                $user = new AppUsers;
                $user->id_role = 2;
                $user->id_perusahaan = $model->id_perusahaan;
                $user->nama_user = $model->nama_perusahaan;
                $user->username = str_replace(' ', '', strtolower($model->nama_perusahaan));
                $user->password = 'testing123';
                $user->password_repeat = 'testing123';
...
// $this->performAjaxValidation($model);
        ...
        if (isset($model->provinsi) && !empty($model->provinsi)) {
            ...
            foreach ($kab as $d) {
                $data['oldp'][] = array('id' => $d->id_kabupaten, 'text' => $d->nama);
            }
        }

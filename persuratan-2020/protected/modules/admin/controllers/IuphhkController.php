...
//        Data Laporan
    ...
    public function actionIndex() {
        ...
        if($role == 1 || $role == 5){  //admin dan opt uhp
            $model = Iuphhk::model()->findByPusat($search_by);
        }
        else if($role == 3){  //bphp
            $model = Iuphhk::model()->findByBphp($userBphp);
        }
        else if($role == 4){  //dishut prop
             $model = Iuphhk::model()->findByPropinsi($userProv);
        }
        else if($role > 6){
             $model = Iuphhk::model()->findByMonevWilayah($role);
        }
        
        
        
        
//        $model = new Iuphhk('search');
//        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Iuphhk']))
            $model->attributes = $_GET['Iuphhk'];
